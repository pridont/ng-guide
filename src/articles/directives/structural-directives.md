...
**მნიშვნელოვანი შენიშვნა:** ამ თავში აღწერილი `NgIf` და `NgFor` დირექტივები
**ძველი მიდგომაა**. მე-17 ვერსიიდან ანგულარს აქვს ჩაშენებული
[Control Flow სინტაქსი](/control-flow) — `@if`, `@for`, `@switch` — რომელიც
ახალ კოდში უნდა გამოიყენოთ. ის უფრო სწრაფია, არაფრის დაიმპორტება არ სჭირდება
და ტაიპსკრიპტთან უკეთ მუშაობს. ეს თავი იმისთვისაა, რომ ძველ პროექტებში
შეხვედრილი კოდი ამოიცნოთ.
...
`NgIf` და `NgFor` `@angular/common`-ის ნაწილია, ამიტომ თუ მათ იყენებთ,
შესაბამისი კლასი კომპონენტის `imports`-ში უნდა დაამატოთ:
...
კიდევ ერთხელ: ახალ კოდში ამათ ნაცვლად [`@if` და `@for`](/control-flow)
გამოიყენეთ. `trackBy` ფუნქციის ადგილს `@for`-ში სავალდებულო `track`
ექსფრეშენი იკავებს.

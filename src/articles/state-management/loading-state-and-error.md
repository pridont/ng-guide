...
თუ ამავე სამ ხაზს `addItem`-შიც ჩავწერთ, კოდი გამეორებას დაიწყებს — და
ყოველ ახალ მეთოდზე ეს გამეორება გაიზრდება. გამოვყოთ ეს საერთო ლოგიკა
...
ახალი სიგნალები მასზე უბრალოდ არსებობს:
...
## რას გვიშველა სიგნალებმა

ღირს ერთი წუთით გაჩერება და იმის შემჩნევა, **რა არ დაგვჭირდა**:

- არ დაგვჭირდა სამი ცალკე `BehaviorSubject` და მათი გეთერები
- არ დაგვჭირდა `tap`, `catchError`, `of` ოპერატორები
- არ დაგვჭირდა `combineLatest`-ით ერთი დიდი "view model" სტრიმის აწყობა
  მხოლოდ იმისთვის, რომ თემფლეითში სამივე მნიშვნელობა ერთდროულად გვქონოდა
- არ დაგვჭირდა `async` ფაიფი და მისი შედეგის `as`-ით ამოღება მთელ თემფლეითზე
- არ დაგვჭირდა `ChangeDetectionStrategy.OnPush`-ის ხელით ჩაწერა

RxJS-ზე დაფუძნებულ ვერსიაში ამ სამი მნიშვნელობის თემფლეითში ერთდროულად
გამოსატანად ცალკე `vm$` სტრიმი იქმნებოდა, რომელიც `combineLatest`-ით
სამივეს აერთიანებდა და შემდეგ `map`-ით ობიექტად აქცევდა — მხოლოდ იმიტომ,
რომ `async` ფაიფი თითოეულ სტრიმზე ცალკე დასუბსქრაიბებას ნიშნავდა.

სიგნალებთან ეს პრობლემა საერთოდ არ არსებობს: სიგნალს ყოველთვის აქვს
მიმდინარე მნიშვნელობა, ამიტომ თემფლეითში მათი თავისუფლად შერევა შეიძლება.

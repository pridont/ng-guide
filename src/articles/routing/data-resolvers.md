...
resolver-ის გარეშე ნავიგაცია მაშინვე სრულდება და ცარიელი კომპონენტი
თვითონ ითხოვს მონაცემებს:

```mermaid
sequenceDiagram
    participant R as Router
    participant K as კომპონენტი
    participant S as სერვერი

    R->>K: ნავიგაცია
    K->>S: მოთხოვნა
    Note over K: ცარიელი ეკრანი
    S-->>K: მონაცემები
```

resolver-ით კი ნავიგაცია **ელოდება** მონაცემებს, და კომპონენტი მხოლოდ
შემდეგ იქმნება:

```mermaid
sequenceDiagram
    participant R as Router
    participant G as canActivate
    ...
    S-->>Rs: მონაცემები
    Rs-->>R: მონაცემები
    R->>K: შექმნა მონაცემებით
```

ყურადღება მიაქციეთ რიგს: ჯერ გარდი წყვეტს, გავიდეთ თუ არა, და მხოლოდ
შემდეგ ეშვება resolver — უაზროდ ჩამოტვირთვას ვერიდებით.
